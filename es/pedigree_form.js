...
(function(pedigree_form, $, undefined) {
	...
	$('#acc_FamHist_div').on('click', '#id_proband, #id_exclude', function(e) {
		...
		if($(this).attr("id") === 'id_proband' && $(this).is(':checked')) {
			...
			let dataset = pedcache.current(opts);
            opts.dataset = ptree.copy_dataset(dataset);
			let idx = pedigree_util.getIdxByName(opts.dataset, name);
			if($(this).is(':checked'))
	...
	$(document).on('fhChange', function(e, opts){
		try {
			...
			let node = pedigree_util.getNodeByName(pedcache.current(opts), id)
			if(node === undefined)
...
/*		if('ashkenazi' in node) {
    ...
    pedigree_form.save = function(opts) {
		...
		let name = $('#id_name').val();
		let newdataset = ptree.copy_dataset(dataset);
		let person = pedigree_util.getNodeByName(newdataset, name);
		if(!person) {
