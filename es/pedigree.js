...
(function(pedigree_util, $, undefined) {
	...
	pedigree_util.buildTree = function(opts, person, root, partnerLinks, id) {
		...
		let nodes = pedigree_util.flatten(root);
		...
		$.each(person.children, function(i, child) {
			$.each(opts.dataset, function(j, p) {
				if (((child.name === p.mother) || (child.name === p.father)) && child.id === undefined) {
					let m = pedigree_util.getNodeByName(nodes, p.mother);
					let f = pedigree_util.getNodeByName(nodes, p.father);
		...
		$.each(partners, function(i, ptr) {
			...
			let midx = pedigree_util.getIdxByName(opts.dataset, mother.name);
			let fidx = pedigree_util.getIdxByName(opts.dataset, father.name);
			if(!('id' in father) && !('id' in mother))
				id = setChildrenId(person.children, id);

			// look at grandparents index
			let gp = pedigree_util.get_grandparents_idx(opts.dataset, midx, fidx);
			if(gp.fidx < gp.midx) {
				father.id = id++;
				parent.id = id++;
				mother.id = id++;
	...
	function updateParent(p, parent, id, nodes, opts) {
		...
		if(p.mztwin || p.dztwins) {
			let twins = pedigree_util.getTwins(opts.dataset, p);
			for(let i=0; i<twins.length; i++) {
				let twin = pedigree_util.getNodeByName(nodes, twins[i].name);
				if(twin)
					twin.id = id++;
			}
		}
	...
	pedigree_util.getTwins = function(dataset, person) {
		let sibs = pedigree_util.getSiblings(dataset, person);
	...
	pedigree_util.consanguity = function(node1, node2, opts) {
		if(node1.depth !== node2.depth) // parents at different depths
			return true;
		let ancestors1 = pedigree_util.ancestors(opts.dataset, node1);
		let ancestors2 = pedigree_util.ancestors(opts.dataset, node2);
		...
		function recurse(node) {
			if (node.children) {
				...
				if(node.data.father !== undefined) { 	// hidden nodes
					let father = pedigree_util.getNodeByName(flattenNodes, node.data.father.name);
					let mother = pedigree_util.getNodeByName(flattenNodes, node.data.mother.name);
	...
	pedigree_util.proband_attr = function(opts, keys, value){
		let proband = opts.dataset[ pedigree_util.getProbandIndex(opts.dataset) ];
	...
	pedigree_util.node_attr = function(opts, name, keys, value){
		...
		let node = pedigree_util.getNodeByName(newdataset, name);
		if(!node){
	...
	pedigree_util.delete_node_by_name = function(opts, name){
		...
		let newdataset = ptree.copy_dataset(pedcache.current(opts));
		let node = pedigree_util.getNodeByName(pedcache.current(opts), name);
		if(!node){
...
/*		let warn = node.filter(function (d) {
		...
		partners = ped.selectAll(".partner")
		  	...
		  	.enter()
		  		...
		  		.attr('d', function(d, i) {
		  			...
		  			let clash = ptree.check_ptr_link_clashes(opts, d);
		  			let path = "";
		  			if(clash) {
						...
						parent_node.y = dy1; // adjust hgt of parent node
		  				...
		  				let dy2 = (dy1-opts.symbol_size/2-3);
		...
		if(typeof probandIdx !== 'undefined') {
			...
			let triid = "triangle"+ptree.makeid(3);
	...
	ptree.unconnected = function(dataset){
		let target = dataset[ pedigree_util.getProbandIndex(dataset) ];
		if(!target){
	...
	function include_children(connected, p, dataset) {
		...
		let children = pedigree_util.getAllChildren(dataset, p);
	...
	function check_ptr_links(opts, ptrLinkNodes){
		for(let a=0; a<ptrLinkNodes.length; a++) {
			let clash = ptree.check_ptr_link_clashes(opts, ptrLinkNodes[a]);
			if(clash)
	...
	ptree.check_ptr_link_clashes = function(opts, anode) {
		let root = ptree.roots[opts.targetDiv];
	...
	ptree.addchild = function(dataset, node, sex, nchild, twin_type) {
		...
		if (children.length === 0) {
			let partner = ptree.addsibling(dataset, node, node.sex === 'F' ? 'M': 'F', node.sex === 'F');
		...
		for (let i = 0; i < nchild; i++) {
			let child = {"name": ptree.makeid(4), "sex": sex,
	...
	ptree.addsibling = function(dataset, node, sex, add_lhs, twin_type) {
		...
		let newbie = {"name": ptree.makeid(4), "sex": sex};
		if(node.top_level) {
	...
	ptree.addpartner = function(opts, dataset, name) {
		...
		let partner = ptree.addsibling(dataset, tree_node.data, tree_node.data.sex === 'F' ? 'M' : 'F', tree_node.data.sex === 'F');
		partner.noparents = true;

		let child = {"name": ptree.makeid(4), "sex": "M"};
	...
	ptree.delete_node_dataset = function(dataset, node, opts, onDone) {
		...
		try	{
			...
			unconnected = ptree.unconnected(dataset);
		} catch(err) {
