 ...
 * License: GPL-3.0
 */
var pedigreejs = (function (exports, $, d3) {
	'use strict';
	...
	**/
	...
	function init_zoom(opts, svg) {
	  ...
	  zm = d3__namespace.zoom().scaleExtent([opts.zoomIn, opts.zoomOut]).filter(function (e) {
	  ...
	  var transform = d3__namespace.zoomIdentity.scale(k).translate(x, y);
	  svg.call(zm.transform, transform);
	}

	// scale size the pedigree
	function btn_zoom(opts, scale) {
	  let svg = d3__namespace.select("#" + opts.targetDiv).select("svg");
	...
	function zooming(e, opts) {
	  ...
	  let ped = d3__namespace.select("#" + opts.targetDiv).select(".diagram");
	...
	function getNodeSize(opts, g_elm, sym) {
	  let node = d3__namespace.select(g_elm).node();
	  let dg = node.getBBox();
	  let w = dg.width;
	  let h = dg.height;
	  if (w === 0 && h === 0) {
	    ...
	    try {
	      w = sym * 2;
	      h = sym * 2;
	      let text_elements = d3__namespace.select(g_elm).selectAll(".indi_details"); // get individuals details
	...
	**/
	...
	function get_svg_as_data_url(svg) {
	  ...
	  d3__namespace.select(svgCopy).selectAll("text").filter(function () {
	    return d3__namespace.select(this).text().length === 0 || d3__namespace.select(this), d3__namespace.select(this).attr('font-family') === "FontAwesome";
	  }).remove();
	...
	function copy_svg(opts) {
	  ...
	  let d3obj = d3__namespace.select(svg_node.get(0));
	...
	**/
	...
	function addWidgets(opts, node) {
	  ...
	  let popup_selection = d3__namespace.select('.diagram');
	  ...
	  d3__namespace.selectAll(".persontype").on("click", function () {
	    let newdataset = copy_dataset(current(opts));
	    let mztwin = d3__namespace.select(this).classed("mztwin");
	    let dztwin = d3__namespace.select(this).classed("dztwin");
	    let twin_type;
	    let sex;
	    if (mztwin || dztwin) {
	      ...
	      sex = d3__namespace.select(this).classed("fa-square") ? 'M' : d3__namespace.select(this).classed("fa-circle") ? 'F' : 'U';
	    }
	    ...
	    opts.dataset = newdataset;
	    $(document).trigger('rebuild', [opts]);
	    d3__namespace.selectAll('.popup_selection').attr("opacity", 0);
	    add_person = {};
	  }).on("mouseover", function () {
	    if (add_person.node) add_person.node.select('rect').attr("opacity", 0.2);
	    d3__namespace.selectAll('.popup_selection').attr("opacity", 1);
	    // add tooltips to font awesome widgets
	    if (add_person.type === 'addsibling') {
	      if (d3__namespace.select(this).classed("fa-square")) square_title.text("add brother");else circle_title.text("add sister");
	    } else if (add_person.type === 'addchild') {
	      if (d3__namespace.select(this).classed("fa-square")) square_title.text("add son");else circle_title.text("add daughter");
	    }
	  });

	  // handle mouse out of popup selection
	  d3__namespace.selectAll(".popup_selection").on("mouseout", function () {
	  ...
	  for (let key in widgets) {
	    let widget = node.filter(function (d) {
	      let motherName = typeof d.data.mother === "string" ? d.data.mother : d.data.mother?.name;
	      let mother = opts.dataset.find(item => item.name === motherName);
	  ...
	  d3__namespace.selectAll(".addsibling, .addchild").on("mouseover", function () {
	    let type = d3__namespace.select(this).attr('class');
	    d3__namespace.selectAll('.popup_selection').attr("opacity", 1);
	    add_person = {
	      'node': d3__namespace.select(this.parentNode),
	      'type': type
	    };

	    //let translate = getTranslation(d3.select('.diagram').attr("transform"));
	    let x = parseInt(d3__namespace.select(this).attr("xx")) + parseInt(d3__namespace.select(this).attr("x"));
	    let y = parseInt(d3__namespace.select(this).attr("yy")) + parseInt(d3__namespace.select(this).attr("y"));
	    d3__namespace.selectAll('.popup_selection').attr("transform", "translate(" + x + "," + (y + 2) + ")");
	    d3__namespace.selectAll('.popup_selection_rotate45').attr("transform", "translate(" + (x + 3 * font_size) + "," + (y + font_size * 1.2) + ") rotate(45)");
	  });

	  // handle widget clicks
	  d3__namespace.selectAll(".addchild, .addpartner, .addparents, .delete, .settings").on("click", function (e) {
	    e.stopPropagation();
	    let opt = d3__namespace.select(this).attr('class');
	    let d = d3__namespace.select(this.parentNode).datum();
	    if (opts.DEBUG) {
	      console.log(opt);
	    }
	    let newdataset;
	  ...
	  node.filter(function (d) {
	    ...
	    if ('nodeclick' in opts) {
	      ...
	      d3__namespace.selectAll(".indi_rect").attr("opacity", 0);
	      d3__namespace.selectAll('.indi_rect').filter(function (d) {
	    ...
	    if (dragging) {
	      if (dragging.data.name !== last_mouseover.data.name && dragging.data.sex !== last_mouseover.data.sex && last_mouseover.data.sex !== "U" && dragging.data.sex !== "U") {
	        d3__namespace.select(this).select('rect').attr("opacity", 0.2);
	      }
	      return;
	    }
	    d3__namespace.select(this).select('rect').attr("opacity", 0.2);
	    d3__namespace.select(this).selectAll('.addchild, .addsibling, .addpartner, .addparents, .delete, .settings').attr("opacity", 1);
	    d3__namespace.select(this).selectAll('.indi_details').attr("opacity", 0);
	    setLineDragPosition(opts.symbol_size - 10, 0, opts.symbol_size - 2, 0, d.x + "," + (d.y + 2));
	  }).on("mouseout", function (d) {
	    if (dragging) return;
	    d3__namespace.select(this).selectAll('.addchild, .addsibling, .addpartner, .addparents, .delete, .settings').attr("opacity", 0);
	    if (highlight.indexOf(d) === -1) d3__namespace.select(this).select('rect').attr("opacity", 0);
	    d3__namespace.select(this).selectAll('.indi_details').attr("opacity", 1);
	    // hide popup if it looks like the mouse is moving north
	    let xcoord = d3__namespace.pointer(d)[0];
	    let ycoord = d3__namespace.pointer(d)[1];
	    if (ycoord < 0.8 * opts.symbol_size) d3__namespace.selectAll('.popup_selection').attr("opacity", 0);
	    if (!dragging) {
	...
	function drag_handle(opts) {
	  let line_drag_selection = d3__namespace.select('.diagram');
	  let dline = line_drag_selection.append("line").attr("class", 'line_drag_selection').attr("stroke-width", 6).attr("stroke-dasharray", "2, 1").attr("stroke", "black").call(d3__namespace.drag().on("start", dragstart).on("drag", drag).on("end", dragstop));
	  dline.append("svg:title").text("drag to create consanguineous partners");
	  setLineDragPosition(0, 0, 0, 0);
	  function dragstart() {
	    dragging = last_mouseover;
	    d3__namespace.selectAll('.line_drag_selection').attr("stroke", "darkred");
	  }
	  function dragstop(_d) {
	    ...
	    setLineDragPosition(0, 0, 0, 0);
	    d3__namespace.selectAll('.line_drag_selection').attr("stroke", "black");
	    dragging = undefined;
	    return;
	  }
	  function drag(e) {
	    ...
	    let dx = e.dx;
	    let dy = e.dy;
	    let xnew = parseFloat(d3__namespace.select(this).attr('x2')) + dx;
	    let ynew = parseFloat(d3__namespace.select(this).attr('y2')) + dy;
	...
	function setLineDragPosition(x1, y1, x2, y2, translate) {
	  if (translate) {
	    d3__namespace.selectAll('.line_drag_selection').attr("transform", "translate(" + translate + ")");
	  }
	  d3__namespace.selectAll('.line_drag_selection').attr("x1", x1).attr("y1", y1).attr("x2", x2).attr("y2", y2);
	}
	...
	**/
	...
	function addLabels(opts, node) {
	  ...
	  for (let i = 0; i < opts.diseases.length; i++) {
	    ...
	    addLabel(opts, node, -opts.symbol_size, function (d) {
	      ...
	      if (!disp) disp = disease.replace('_', ' ').replace('cancer', 'ca.');
	...
	function init_dragging(opts, node) {
	  ...
	  node.filter(function (d) {
	    return !d.data.hidden;
	  }).call(d3__namespace.drag().filter(function filter(event) {
	    return !event.ctrlKey && !event.button && event.shiftKey; // shift and drag
	  }).on("start", dragstart).on("drag", drag).on("end", dragstop));
	  let xstart;
	  let xnew;
	  function dragstart() {
	    xstart = d3__namespace.select(this).select('.indi_rect').attr('x');
	  }
	  function drag(e) {
	    e.sourceEvent.stopPropagation();
	    let dx = e.dx;
	    let indir = d3__namespace.select(this).select('.indi_rect');
	    xnew = parseFloat(indir.attr('x')) + dx;
	    indir.attr("x", xnew);
	  }
	  function dragstop(_d) {
	    let me = d3__namespace.select(this).datum().data;
	...
	**/
	...
	function build(options) {
	  ...
	  let svg_dimensions = get_svg_dimensions(opts);
	  let svg = d3__namespace.select("#" + opts.targetDiv).append("svg:svg").attr("width", svg_dimensions.width).attr("height", svg_dimensions.height);
	  ...
	  let root = d3__namespace.hierarchy(hidden_root);
	  ...
	  let treemap = d3__namespace.tree().separation(function (a, b) {
	  ...
	  }).attr("d", d3__namespace.symbol().size(function (_d) {
	    return opts.symbol_size * opts.symbol_size + 2;
	  }).type(function (d) {
	    if (d.data.miscarriage || d.data.termination) return d3__namespace.symbolTriangle;
	    return d.data.sex === "F" ? d3__namespace.symbolCircle : d3__namespace.symbolSquare;
	  ...
	  node.filter(function (d) {
	    ...
	    return !has_gender(d.data.sex) && !(d.data.miscarriage || d.data.termination) ? "rotate(45)" : "";
	  }).attr("d", d3__namespace.symbol().size(function (d) {
	  ...
	  pienode.selectAll("path").data(d3__namespace.pie().value(function (d) {
	    return d.cancer;
	  })).enter().append("path").attr("clip-path", function (d) {
	    return "url(#" + d.data.id + ")";
	  }) // clip the rectangle
	  .attr("class", "pienode").attr("d", d3__namespace.arc().innerRadius(0).outerRadius(opts.symbol_size)).attr("fill", function (d, i) {
	  ...
	  ped.selectAll(".partner").data(ptrLinkNodes).enter().insert("path", "g").attr("fill", "none").attr("stroke", "#000").attr("shape-rendering", "auto").attr('d', function (d, _i) {
...
//# sourceMappingURL=pedigreejs.v5.0.1.js.map
