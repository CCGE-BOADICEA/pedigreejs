var pedigreejs = (function (exports) {
	...
	**/
	...
	let max_limit = 25;
	let dict_cache = {}; // test if browser storage is supported
	...
	**/
	...
	function buildTree(opts, person, root, partnerLinks, id) {
	  ...
	  let nodes = flatten(root); //console.log('NAME='+person.name+' NO. CHILDREN='+person.children.length);
	...
	function node_attr(opts, name, keys, value) {
	  ...
	  if (value) {
	    for (let i = 0; i < keys.length; i++) {
	      let k = keys[i]; //console.log('VALUE PROVIDED', k, value, (k in node));
	    ...
	    for (let i = 0; i < keys.length; i++) {
	      let k = keys[i]; //console.log('NO VALUE PROVIDED', k, (k in node));
	...
	/* SPDX-License-Identifier: GPL-3.0-or-later
	**/
	let zm; // initialise zoom and drag
	...
	**/
	...
	let pathology_tests = ['er', 'pr', 'her2', 'ck14', 'ck56']; // risk factor to storage

	let RISK_FACTOR_STORE = new Object(); // get surgical ops and PRS for canrisk header
	...
	function get_pedigree(dataset, famid, meta, isanon, version = 2) {
	...
	**/
	...
	function copy_svg(opts) {
	  ...
	  let d3obj = d3.select(svg_node.get(0)); // remove unused elements
	...
	function readBoadiceaV4(boadicea_lines, version) {
	  let lines = boadicea_lines.trim().split('\n');
	  let ped = []; // assumes two line header
	  ...
	  for (let i = 0; i < ped.length; i++) {
	    if (getDepth(ped, ped[i].name) == 1) {
	      if (ped[i].level && ped[i].level == max_level) {
	        ...
	        ped[i].noparents = true; // 1. look for partners parents
	...
	**/
	...
	function addWidgets(opts, node) {
	  ...
	  let add_person = {}; // click the person type selection
	...
	**/
	function addLabels(opts, node) {
	  ...
	  let font_size = parseInt(getPx(opts)) + 4; // display age/yob label first
	  ...
	  for (let l = 0; l < arr.length; l++) {
	    ...
	    if (d.data[this_label]) {
	      if (this_label === 'alleles') {
	        ...
	        let r = d.data[this_label]['result'].toUpperCase(); //let t = d.data[this_label]['type'];
	...
	**/
	...
	function build(options) {
	  ...
	  let flattenNodes = nodes.descendants(); // check the number of visible nodes equals the size of the pedigree dataset
	  ...
	  let clash_depth = {}; // get path looping over node(s)
	  ...
	  partners = ped.selectAll(".partner").data(ptrLinkNodes).enter().insert("path", "g").attr("fill", "none").attr("stroke", "#000").attr("shape-rendering", "auto").attr('d', function (d, _i) {
	    ...
	    let dy1 = d.mother.y;
	    let dy2, dx, parent_node; // identify clashes with other nodes at the same depth
	  ...
	  ped.selectAll(".link").data(root.links(nodes.descendants())).enter().filter(function (d) {
	    ...
	    if (d.target.data.mztwin || d.target.data.dztwin) {
	      ...
	      if (twins.length >= 1) {
	        let twinx = 0;
	        let xmin = d.target.x; //let xmax = d.target.x;
	...
	function check_ptr_link_clashes(opts, anode) {
	  ...
	  let dy = mother.y; // identify clashes with other nodes at the same depth
	...
	function get_tree_dimensions(opts) {
	  ...
	  for (let i = 0; i < opts.dataset.length; i++) {
	    ...
	    let children = getAllChildren(opts.dataset, opts.dataset[i]); // score based on no. of children and if parent defined
	...
	function addparents(opts, dataset, name) {
	  ...
	  if (depth == 1) {
	    ...
	    let node_sibs = getAllSiblings(dataset, node); // lhs & rhs id's for siblings of this node
	...
	function delete_node_dataset(dataset, node, opts, onDone) {
	  ...
	  if (node.parent_node) {
	    for (i = 0; i < node.parent_node.length; i++) {
	      ...
	      let ps = [getNodeByName(dataset, parent.mother.name), getNodeByName(dataset, parent.father.name)]; // delete parents
